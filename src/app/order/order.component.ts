...
export class OrderComponent implements OnInit {
  IMG_URL = `http://45.114.141.166:5500/products/` + `${this.ls.currentDiv.db.replace('bak_', '')}` + `/`;
  ...
  itemSequenceIdx: number = 0;
  itemSequence: any = [];
  selectedItemSequence: any = {};
  ...
  getCategories() {
    ...
    // this.ds.getData('jwm_getCategories', 0, this.ls.currentDiv.db)
    // .subscribe({
    //   next: data => {
  ...
  getProductSequence() {
    ...
    .subscribe({
      next: data => {
        this.itemSequenceIdx = 1;
        this.itemSequence = data;
        this.selectedItemSequence = this.itemSequence[0];

        this.getProductWithColors();
      },
      error: err => {
        alert(JSON.stringify(err));
      }
  ...
  getProductWithColors() {
    this.loading.items = true;
    this.orderItemForm.disable();
    this.itemIdx = 0;

    this.ds.getData('jwm_getItemSequence', {prodName: this.selectedItemSequence.prodName}, this.ls.currentDiv.db)
    .subscribe({
      next: data => {
        this.items = data;
        this.bindItems(this.itemIdx);
  ...
  bindItems(idx: number) {
    ...
    this.orderItemForm.controls.color.setValue(this.selectedItem.color);

    const qty = this.ls.currentDiv.setQty;
    if (this.selectedItem.qty1 != 0) {this.orderItemForm.controls.qty1.setValue(qty)}
    if (this.selectedItem.qty2 != 0) {this.orderItemForm.controls.qty2.setValue(qty)}
    if (this.selectedItem.qty3 != 0) {this.orderItemForm.controls.qty3.setValue(qty)}
    if (this.selectedItem.qty4 != 0) {this.orderItemForm.controls.qty4.setValue(qty)}
    if (this.selectedItem.qty5 != 0) {this.orderItemForm.controls.qty5.setValue(qty)}
    if (this.selectedItem.qty6 != 0) {this.orderItemForm.controls.qty6.setValue(qty)}
    if (this.selectedItem.qty7 != 0) {this.orderItemForm.controls.qty7.setValue(qty)}
    if (this.selectedItem.qty8 != 0) {this.orderItemForm.controls.qty8.setValue(qty)}
    if (this.selectedItem.qty9 != 0) {this.orderItemForm.controls.qty9.setValue(qty)}
    if (this.selectedItem.qty10 != 0) {this.orderItemForm.controls.qty10.setValue(qty)}
    if (this.selectedItem.qty11 != 0) {this.orderItemForm.controls.qty11.setValue(qty)}
    if (this.selectedItem.qty12 != 0) {this.orderItemForm.controls.qty12.setValue(qty)}
    this.getItemTotQty();
  ...
  next = () => {
    if (this.itemSequenceIdx != this.itemSequence.length) {
      this.itemSequenceIdx++;
      const idx = this.itemSequence.findIndex(x => x.rowID == this.itemSequenceIdx);
      this.selectedItemSequence = this.itemSequence[idx];
  ...
  addItem() {
    ...
    .subscribe({
      next: data => { 
        ...
        if (this.items.length != (this.itemIdx + 1)) {
          this.itemIdx++;
          this.bindItems(this.itemIdx);
        } else {
          this.next();
